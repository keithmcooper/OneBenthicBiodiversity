################################################################################
######### RANDOM FOREST MODELS - DIVERSITY CLUSTERS   ##########################
######### 11/09/2025                                  ##########################
################################################################################
...
## Response data file
## Column 1: 'BD Cluster' Cluster class (character)
## Column 2: 'x' Longitude (dd.00)
## Column 3: 'y' Latitude (dd.00)      
rd = 'biodiv_cluster_4_modelling.csv'
...
response <- as.data.table(read.csv(paste0(in_path,rd))) %>%
  filter(paper=='biodiversity', metric=='biodiversity_cluster') %>%
  mutate_at(c('value'), as.factor) %>%
  select(value,x,y) %>%
  rename(c(`BD Cluster`='value'))
summary(response)
...
save(pshp,pshp_sf,s1,envlab,prnames,facvars,numvars,rspnames, 
     file = paste0(out_path,'BiodiversityClusterModelData.RData'))
...
plotdata <- plotdata %>%
              ...
              levels = c('2','8','5','1','4','6','7','3'),
              labels = c("Bio-A","Bio-B","Bio-C","Bio-D","Bio-E",
                         "Bio-F","Bio-G","Bio-H")))
...
for (j in 1:10){
  ...
  for (p in 1:length(predsel)) {
    ...
    for (c in levels(mdata$resp)) {
      ...
      predname <- predsel[p]
      temp <- data.frame(Name='BD Cluster',predvar=predsel[p],class=c,x=pdata[[1]],y=pdata[[2]])
      plotdata <- rbind(plotdata,temp)
    ...
    for (f in 1:length(facvars)){
      ...
      for (c in levels(mdata$resp)) {
        ...
        pdata <- partial(ffs[[j]],pred.var = facvars[f],which.class = c,
                         plot = FALSE,train=mdata,grid.resolution=50,prob = TRUE)
        predname <- facvars[j]
        temp <- data.frame(Name='BD Cluster',predvar=facvars[f],class=c,x=pdata[[1]],y=pdata[[2]])
...
classrestot <- data.frame('BD Cluster',1:length(SPE),"Overall",nrow(train),SEN,SPE,BA)
...
plotterBC <- classvalB %>%
              ...
              mutate(variable = factor(variable, 
                                       labels = c("Sensitivity", "Specificity",
                                                  "Balanced Accuracy")),
                     Class= factor(Class,
                                   levels = c('2','8','5','1','4','6','7','3','Overall'),
                                   labels = c("Bio-A","Bio-B","Bio-C","Bio-D","Bio-E",
                                              "Bio-F","Bio-G","Bio-H",'Overall'))
                                   )
...
res.mat.num2 <- res.mat.all %>%
  filter(Pred!="U" & Obs!="P") %>%
  mutate(across(c(Pred,Obs), ~ factor(., levels = c('2','8','5','1','4','6','7','3')))) %>%
  mutate(bio_class_p = case_match(Pred,
                                  '2' ~ "Bio-A",
                                  '8' ~ "Bio-B",
                                  '5' ~ "Bio-C",
                                  '1' ~ "Bio-D",
                                  ...
                                  '7' ~ "Bio-G",
                                  '3' ~ "Bio-H")) %>%
  mutate(bio_class_o = case_match(Obs,
                                  '2' ~ "Bio-A",
                                  '8' ~ "Bio-B",
                                  '5' ~ "Bio-C",
                                  '1' ~ "Bio-D",
...
## Highlight classes
highlights <- bgvals2 %>%
  group_by(bio_class_p,bio_class_o) %>%
...
confmtx <-  ggplot(bgvals2) +
  ...
  facet_grid2(bio_class_p~bio_class_o, scales = 'free',strip = strip) +
  xlab("OBSERVED") +
  ylab("PREDICTED") +
  theme_swap_y() +
...
impplot <-  ggplot(imppl,(aes(x=Var,y=Mean))) +
  ...
  geom_bar(stat = 'identity',fill=colpal[2], col=colpal[1],width = 0.8) +
  scale_x_discrete(labels = function(x) sapply(x, function(i) label_map[[i]])) +
...
pplotdata <- plotdata %>%
  ...
  mutate(bio_class = case_match(class,
                                '2' ~ "Bio-A",
                                '8' ~ "Bio-B",
                                '5' ~ "Bio-C",
                                '1' ~ "Bio-D",
...
for (i in top6) {
  ...
  cvRP.list[[i]] <- ggplot(pdata,aes(x=x,y=y,col=bio_class)) +
